...
export function AuthScreenEnhanced({ mode }: AuthScreenProps) {
  ...
  const passwordLabel = passwordStrength <= 1 ? "Weak" : passwordStrength <= 3 ? "Moderate" : "Strong";
  const passwordBarClass = passwordStrength <= 1 ? "bg-destructive" : passwordStrength <= 3 ? "bg-warning" : "bg-success";
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh auth session state before navigating to ensure dashboard recognizes the user
      await refresh();
      navigate("/");
    } catch (error) {
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-3rem)] max-w-7xl items-center gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="glass rounded-[2rem] p-6 shadow-[var(--shadow-card)] sm:p-8 lg:p-10">
          ...
          <form className="space-y-5" onSubmit={submit}>
            ...
            {isLogin ? (
              <label className="flex items-center gap-3 rounded-xl border border-border/60 bg-background/50 px-4 py-3 text-sm">
              ...
              <>
                ...
                <div className="rounded-2xl border border-border/60 bg-background/50 p-4">
                  <div className="mb-2 flex items-center justify-between text-xs font-mono uppercase tracking-[0.18em] text-muted-foreground">
                    <span>Password strength</span>
                    <span>{passwordLabel}</span>
                  </div>
                  <div className="h-2 overflow-hidden rounded-full bg-muted">
                    <div
                      className={`h-full rounded-full transition-all ${passwordBarClass}`}
                      style={{ width: `${(passwordStrength / 5) * 100}%` }}
                    />
                  </div>
                  <div className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-2">
                    {passwordRules.map(rule => (
                      <div key={rule.label} className={rule.passed ? "text-success" : "text-muted-foreground"}>
